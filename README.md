[![Build Status](https://travis-ci.org/leanprover/programming_in_lean.svg?branch=master)](https://travis-ci.org/leanprover/programming_in_lean)

Programming in Lean
===================

 - Web : https://leanprover.github.io/programming_in_lean
...
We use [cask][cask] to install emacs dependencies ([org-mode][org-mode], [lean-mode][lean-mode], [htmlize][htmlize]). We assume that you already have emacs-24.3 or higher installed in your system.
...
git clone https://github.com/leanprover/programming_in_lean
cd programming_in_lean
...
```bash
programming_in_lean $ python -m SimpleHTTPServer
```

The above command starts a HTTP server at `programming_in_lean` directory (default port: `8000`). For example, `01_Introduction.html` is available at `http://localhost:8000/01_Introduction.html`.
